...
class SwinTransformerOD(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
...
def get_swin_model(arch='', pretrained=False):
    for k, v in configs.items():
        if arch.startswith(k):
            ...
            if pretrained:
                ...
                if arch in checkpoint_urls:
                    checkpoint_path = checkpoint_urls[arch]
                else:
                    url_keys = [_k for _k in checkpoint_urls.keys() if _k.startswith(arch)]
                    if len(url_keys) <= 0:
                        raise ValueError('arch [{}] not found in checkpoint_urls'.format(arch))
                    else:
                        for _subfix in ['_22k', '_22kto1k']:
                            for _url_key in url_keys:
                                if _url_key.endswith(_subfix):
                                    checkpoint_path = checkpoint_urls[_url_key]
                                    break
                            if checkpoint_path is not None:
                                break
                        if checkpoint_path is None:
                            checkpoint_path = checkpoint_urls[url_keys[0]]
    ...
    raise ValueError(f'arch [{arch}] not found! Use one of {list(configs.keys())}')
...
def get_swin_model_od(arch='', pretrained=False):
    for k, v in configs.items():
        if arch.startswith(k):
            model = SwinTransformerOD(
                num_classes=0,
                **v,
            )
    ...
    if pretrained:
        raise ValueError(f'arch [{arch}] not found! Use one of {list(checkpoint_urls.keys())}')
    else:
        raise ValueError(f'arch [{arch}] not found! Use one of {list(configs.keys())}')
